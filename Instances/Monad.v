...
Section AbstractMaybe_Monad.
  ...
  Proof.
    unfold bind_maybeA; destruct x; unfold compose; reflexivity.
  Qed.
  ...
  Proof. 
    intros. unfold app. simpl. simple_solve.
  Qed.
...
Section MaybeT_Applicative.
  ...
  Lemma app_maybeT_id : ∀ {A} (f : M (Maybe A)),
  ...
  Proof. 
    ...
    autorewrite with soundness. f_equal. ext. destruct x0; reflexivity.
  Qed.
...
Definition NoneT {M A} `{Monad M} : MaybeT M A := pure None.
Hint Unfold NoneT : soundness.
Definition JustT {M A} `{Monad M} (a : A) : MaybeT M A := pure (Just a).
...
Section MaybeT_MonadT.
  ...
  Definition lift_maybeT {A} (Ma : M A) : M (Maybe A) :=
    fmap Just Ma.
...
Section MaybeAT_Applicative.
  ...
  Definition app_maybeAT {A B} (Mmf : MaybeAT M (A -> B))
    ...
    @bindM M _ _ _ Mmf 
      ...
      | JustOrNoneA f => @bindM M _ _ _ Mma (λ ma : AbstractMaybe A,
          match ma with
...
Section MaybeAT_Monad.
  ...
  Proof.
    intros. unfold bind_maybeAT. unfold app; simpl; unfold app_maybeAT.
    autorewrite with soundness. f_equal; ext. destruct x; autorewrite with
    soundness. f_equal; ext. destruct x. unfold pure_maybeAT.
...
Section MaybeAT_MonadT.
  ...
  Proof. 
    ...
    autorewrite with soundness. simpl. unfold bind_maybeAT. autorewrite with
      soundness. f_equal; ext. unfold compose. autorewrite with soundness.
