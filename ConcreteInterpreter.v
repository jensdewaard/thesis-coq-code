...
Fixpoint eval_expr (e : expr) : State cvalue :=
  ...
  | EPlus e1 e2 => 
      ...
      returnM (VNat (plus_op n1 n2))
  | EMult e1 e2 =>
      v1 << (eval_expr e1) ;
      v2 << (eval_expr e2) ;
      n1 << (ensure_numerical v1) ;
      n2 << (ensure_numerical v2) ;
      returnM (VNat (mult_op n1 n2))
  | EEq e1 e2 =>
      v1 << (eval_expr e1) ;
      v2 << (eval_expr e2) ;
      n1 << (ensure_numerical v1) ;
      n2 << (ensure_numerical v2) ;
      returnM (VBool (eq_op n1 n2))
  | ELe e1 e2 =>
      v1 << (eval_expr e1) ;
      v2 << (eval_expr e2) ;
      n1 << (ensure_numerical v1) ;
      n2 << (ensure_numerical v2) ;
      returnM (VBool (le_op n1 n2))
  | ENot e =>
      v << (eval_expr e) ;
      b << (ensure_boolean v) ;
      returnM (VBool (neg_op b))
  | EAnd e1 e2 =>
      v1 << (eval_expr e1) ;
      v2 << (eval_expr e2) ;
      b1 << (ensure_boolean v1) ;
      b2 << (ensure_boolean v2) ;
      returnM (VBool (and_op b1 b2))
  end.
